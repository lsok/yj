...
if (isset($_POST['submitted'])) {//如果用户提交了表单则处理用户请求
  ...
  if (isset($_POST['cateid']) && ($_POST['cateid'] == 0)) {//如果是添加分类
	...
	if (IS_CH_EN == 0) { //如果仅为中文版
		...
		if ($catename != '') {
			$query = sprintf('INSERT INTO %simages_cates (FAT_ID, CATENAME) VALUES'.
					'(%d, "%s")',
					DB_TBL_PREFIX,
					$fat_id,
			...
			$query = sprintf('SELECT * FROM %simages WHERE ' .
							'CAT_ID = %d',
			...
			if (isset($have_data) && $have_data = true)
			{
				...
				echo "<script>alert('子类添加成功! 原分类中的数据已自动移入新添加的子类中.');location.href='images_cates.php';</script>";
			}
			else
			{
				echo "<script>location.href='images_cates.php';</script>";
			}
		}
		else {
			echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />'; 
			echo "<script>alert('分类名称不能为空!');location.href='images_cates.php';</script>";
		}
	}
	else 
	{  //如果是中英双语版，同时写入分类英文名称
		...
		$query = sprintf('INSERT INTO %simages_cates (FAT_ID, CATENAME, CATENAME_EN) VALUES'.
				'(%d, "%s", "%s")',
				DB_TBL_PREFIX,
				$fat_id,
				mysql_real_escape_string($catename, $GLOBALS['DB']),
	   ...
	   mysql_query($query, $GLOBALS['DB']) or die(mysql_error($GLOBALS['DB']));
		...
		if ($catename != '') {
			$query = sprintf('UPDATE %simages_cates SET '.
			         'CATENAME = "%s",
					  FAT_ID = %d
					  WHERE 
					  ID = %d',
			...
			echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />'; 
			echo "<script>alert('分类名称不能为空!');location.href='images_cates.php";
...
if (isset($_GET['action']) && ($_GET['action'] == 'del')) {
	...
	//调用递归方法删除分类函数
	del_all_cates($_GET['cateid'], 'images_cates', 'images', 'images_cates.php');
}
?>
...
<?php
//检索所有分类
$query = sprintf('SELECT ID, FAT_ID, CATENAME, CATENAME_EN FROM %simages_cates ORDER BY ID ASC',
				 DB_TBL_PREFIX); 
mysql_query("set names 'utf8'");
$result = mysql_query($query, $GLOBALS['DB']) or die(mysql_error($GLOBALS['DB']));
...
if (mysql_num_rows($result)) {//如果存在分类
	...
	show_cates($father_id = 0, 'images_cates.php', $isen); //调用递归方法输出分类函数
	echo '</div>';
}
else {
...
<?php
//取出所有类别生成选项
$query = sprintf('SELECT ID, CATENAME FROM %simages_cates ORDER BY ID ASC',
				  DB_TBL_PREFIX);
mysql_query("set names 'utf8'");
$result = mysql_query($query, $GLOBALS['DB']) or die(mysql_error($GLOBALS['DB']));
