...
//文件保存目录路径
$save_path = $php_path . '../../../userupload/content/';
//文件保存目录URL
$save_url = $php_url . '../../../userupload/content/';
//定义允许上传的文件扩展名
$ext_arr = array(
...
if (empty($_FILES) === false) {
	...
	if(WATERMARK == 1)
	{
		...
		switch ($type) { 
		case IMAGETYPE_GIF:
			$image = imagecreatefromgif($file_path) or
				die($error);
			break;
		case IMAGETYPE_JPEG:
			$image = imagecreatefromjpeg($file_path) or
				die($error);
			break;
		case IMAGETYPE_PNG:
			$image = imagecreatefrompng($file_path) or
				die($error);
			break;
		default:
			die($error);
		...
		//定义水印图片  "../../../"为网站根目录
		$waterimg="../../../administrator/images/water.png"; 
