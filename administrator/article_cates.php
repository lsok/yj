...
if (isset($_POST['submitted'])) {//如果用户提交了表单则处理用户请求
  ...
  if (isset($_POST['cateid']) && ($_POST['cateid'] == 0)) {//如果是添加分类
	...
	if (IS_CH_EN == 0) { //如果仅为中文版
		...
		if ($catename != '') {
			$query = sprintf('INSERT INTO %sarticles_cates (FAT_ID, CATENAME) VALUES'.
					'(%d, "%s")',
					DB_TBL_PREFIX,
					$fat_id,
			...
			$query = sprintf('SELECT * FROM %sarticles WHERE ' .
							'CAT_ID = %d',
			...
			if (isset($have_data) && $have_data = true)
			{
				...
				echo "<script>alert('子类添加成功! 原分类中的数据已自动移入新添加的子类中.');location.href='article_cates.php';</script>";
			}
			else
			{
				echo "<script>location.href='article_cates.php';</script>";
			}
		}
		else {
			echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />'; 
			echo "<script>alert('分类名称不能为空!');location.href='article_cates.php';</script>";
		}
	}
	else 
	{  //如果是中英双语版，同时写入分类英文名称
		...
		$query = sprintf('INSERT INTO %sarticles_cates (FAT_ID, CATENAME, CATENAME_EN) VALUES'.
				'(%d, "%s", "%s")',
				DB_TBL_PREFIX,
				$fat_id,
				mysql_real_escape_string($catename, $GLOBALS['DB']),
	   ...
	   mysql_query($query, $GLOBALS['DB']) or die(mysql_error($GLOBALS['DB']));
		...
		if ($catename != '') {
			$query = sprintf('UPDATE %sarticles_cates SET '.
			         'CATENAME = "%s",
					  FAT_ID = %d
					  WHERE 
					  ID = %d',
			...
			echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />'; 
			echo "<script>alert('分类名称不能为空!');location.href='article_cates.php";
...
if (isset($_GET['action']) && ($_GET['action'] == 'del')) {
	...
	//调用递归方法删除分类函数
	del_all_cates($_GET['cateid'], 'articles_cates', 'articles', 'article_cates.php');
}
?>
...
function checkdel()
{
	if (confirm("删除分类将同时删除该分类下的数据,确实要删除吗？"))
     {return (true);}
     else
     {return (false);}
}
...
<?php
//检索所有分类
$query = sprintf('SELECT ID, FAT_ID, CATENAME, CATENAME_EN FROM %sarticles_cates ORDER BY ID ASC',
				 DB_TBL_PREFIX); 
mysql_query("set names 'utf8'");
$result = mysql_query($query, $GLOBALS['DB']) or die(mysql_error($GLOBALS['DB']));
...
if (mysql_num_rows($result)) {//如果存在分类
	...
	while ($row = mysql_fetch_assoc($result)) { 
			...
			$arr[] = array($row['ID'],$row['FAT_ID'],$row['CATENAME'],$row['CATENAME_EN']); 
	} 
	
	//输出所有分类
	$isen = IS_CH_EN; // IS_CH_EN为Constant.php中定义的常量 定义是否为中英双语版
	echo '<div class="cates">';
	show_cates($father_id = 0, 'article_cates.php', $isen); //调用递归方法输出分类函数
	echo '</div>';
}
else {
...
<?php
//取出所有类别生成选项
$query = sprintf('SELECT ID, CATENAME FROM %sarticles_cates ORDER BY ID ASC',
				  DB_TBL_PREFIX);
mysql_query("set names 'utf8'");
$result = mysql_query($query, $GLOBALS['DB']) or die(mysql_error($GLOBALS['DB']));
